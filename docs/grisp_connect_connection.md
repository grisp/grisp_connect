...
When performing asynchronous requests, the cller can pass a context term to
...
It expose a higher level API to perform synchronous request, send notification,
send errors, or perform asynchronous requests.
...
```erlang
{request, Conn :: pid(), Method :: method(), Params :: map(), ReqRef :: binary()}
```
...
```erlang
{response, Conn :: pid(), Method :: method(), Result :: jsx:json_term(), ReqCtx :: term()}
```
...
```erlang
{error, Conn :: pid(), Method :: method(), Code :: integer(), Message :: undefined | binary(), Data :: undefined | jsx:json_term(), ReqCtx :: term()}
```
...
```erlang
{timeout, Conn :: pid(), Method :: binary(), ReqCtx :: term()}
```
...
```erlang
{notification, Conn :: pid(), Method :: method(), Params :: jsx:json_term()}
```
...
```erlang
{error, Conn :: pid(), Code :: integer(), Message :: undefined | binary(), Data :: undefined | jsx:json_term()}
```
